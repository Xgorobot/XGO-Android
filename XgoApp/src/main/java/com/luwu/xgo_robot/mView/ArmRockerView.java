...
public class ArmRockerView extends View {

    private static final int DEFAULT_BACKGROUND_SIZE = 400;
    private static final float DEFAULT_ROCKER_RADIUSPERSENT = 0.2f;   // rocker半径
    //设置的属性
    ...
    public ArmRockerView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
...
//                Log.d("Tag", "ACTION_DOWN: ");
                ...
                if ((int) ((mRockerPosition.x - mBackGroundPoint.x) * 100.0f / (mBackgroundRadius - mRockerRadius)) >= xRange || mRockerPosition.y <=mBackGroundPoint.y){
                    moveRocker(mRockerPosition.x, mRockerPosition.y);
                }
                break;
            case MotionEvent.ACTION_UP:  // 不弹回
//                Log.d("Tag", "ACTION_UP: ");
                if (mListener != null) {
                    mListener.actionUp();
                }
